    ...
    try {
      ...
      if (result && result.newCount > 0) {
        ...
        if (settings.notifications) {
          ElMessage.success(`收到 ${result.newCount} 封新邮件`)
        }
      }
    } catch (err) {
...
// 监听 localStorage 变化（其他窗口设置页面保存后）
function handleStorageChange(e: StorageEvent) {
  if (e.key === 'emailAdminSettings') {
    startAutoRefresh() // 重新启动定时器以应用新设置
  }
}
...
async function handleRefresh() {
  await emailStore.refreshEmails(currentAccountId.value || undefined)
  loadEmails()
}
